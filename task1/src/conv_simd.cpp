        ...
        for(int ox=0; ox<W ; ox+=simd_stride){
            ...
            for(int ky=0 ; ky<K ; ++ky){
                for(int kx=0; kx<K ; ++kx){
                    ...
                    acc = _mm256_fmadd_ps(input,kernel_input, acc);
                }
            }
...
//                     acc = _mm_fmadd_ps(input,kernel_input,acc);
//                 }
//             }
...
//                 for(int kx=0; kx<K ; ++kx){
//                     float weight= ker[ky*K+kx];
//                     __m512 input = _mm512_loadu_ps(in + (oy+ky)*in_stride + (ox+kx));
//                     __m512 kernel_input = _mm512_set1_ps(weight);
//                     // __m256 mul_output = _mm256_mul_ps(input, kernel_input);
//                     // acc = _mm256_add_ps(acc, mul_output);
//                     acc = _mm512_fmadd_ps(input,kernel_input, acc);
//                 }
//             }
